...
For our test plan we will add new methods that we did not previously test for this is the easiest way to increase coverage. Then we will move onto imporving the test cases we already have for existing methods. Now that we have the source code we can deliberatley enter different branches and if-else statements in our test cases. Prior to having the source code we weren't able to see what all the conditions were. This way we have now 2 ways of increasing coverage, that is, new methods to test for and improving previous test cases. 
